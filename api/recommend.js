...
function resolveRecommendationContext(userContext={}, analysis={}, localHints={}){
  ...
  // 우선순위 고정:
  // 1) 현재 질문에서 직접 말한 정확한 읍면동
  // 2) 현재 질문에서 직접 말한 화성 권역/생활지명
  // 3) AI가 대화 맥락에서 잡은 집·목적지·가려는 곳
  // 4) 사용자가 선택한 권역
  // 5) GPS 현재 위치
  // 6) 목적 중심
  ...
  if(directArea){
    return {
      userContext:{...userContext, mode:"district", selectedAreaGroup:directArea, basisDistrict:null, lat:undefined, lng:undefined},
      gpsStatus:gps,
  ...
  if(aiDistrict && aiAreaIsContextual){
    ...
    return {
      ...
      gpsStatus:gps,
      usedBasis:aiBasis === "purpose_only" ? "mentioned_district" : aiBasis,
      needsAreaClarification:false,
      areaGroup:area,
      basisDistrict:aiDistrict,
  ...
  if(aiArea && aiAreaIsContextual){
    return {
      userContext:{...userContext, mode:"district", selectedAreaGroup:aiArea, basisDistrict:null, lat:undefined, lng:undefined},
      gpsStatus:gps,
      usedBasis:aiBasis,
      needsAreaClarification:false,
      areaGroup:aiArea,
  ...
  if(selectedArea){
    return {
      userContext:{...userContext, mode:"district", selectedAreaGroup:selectedArea, basisDistrict:null, lat:undefined, lng:undefined},
      gpsStatus:gps,
  ...
  if(gps.hasGps && gps.outsideHwaseong){
    return {
      userContext:{...userContext, mode:"none", basisDistrict:null, lat:undefined, lng:undefined},
      gpsStatus:gps,
  ...
  if(gps.hasGps && gps.inHwaseong){
    return {
      userContext:{...userContext, mode:"gps", basisDistrict:null},
      gpsStatus:gps,
      usedBasis:"gps",
      needsAreaClarification:false,
  ...
  if(aiDistrict){
    ...
    const area = getAreaGroup(aiDistrict);
    return {
      userContext:{...userContext, mode:"district", selectedAreaGroup:area, basisDistrict:aiDistrict, referenceLat:ref?.lat, referenceLng:ref?.lng},
      gpsStatus:gps,
      usedBasis:"mentioned_district",
      needsAreaClarification:false,
      areaGroup:area,
  ...
  if(aiArea){
    return {
      userContext:{...userContext, mode:"district", selectedAreaGroup:aiArea, basisDistrict:null},
      gpsStatus:gps,
      usedBasis:"mentioned_area",
      needsAreaClarification:false,
      areaGroup:aiArea,
  ...
  return {
    userContext:{...userContext, mode:userContext.mode || "none", basisDistrict:null},
    gpsStatus:gps,
...
function getLocationMeta(f, userContext={}){
  ...
  // 사용자가 정확한 읍면동을 말한 경우: 그 읍면동을 최우선 기준으로 보여준다.
  if(userContext.mode === "district" && userContext.basisDistrict){
    ...
    if(Number.isFinite(Number(userContext.referenceLat)) && Number.isFinite(Number(userContext.referenceLng)) && districtCoords[f.district]){
      ...
      if(area === userContext.selectedAreaGroup) return { type:"sameArea", label:`${userContext.selectedAreaGroup} 내 시설`, detail:`${userContext.basisDistrict} 인근 동탄권 시설`, distanceKm:Number(km.toFixed(1)) };
